...
class FlickVideoManager extends ChangeNotifier {
  ...
  bool get errorInVideo =>
      cachedVideoPlayerController?.value?.hasError ?? false;

  /// Is current video initialized.
  bool get isVideoInitialized =>
      cachedVideoPlayerController?.value?.isInitialized ?? false;
  ...
  _changeVideo(CachedVideoPlayerController newController) async {
    ...
    if (!cachedVideoPlayerController.value.isInitialized && autoInitialize) {
      try {
  ...
  _videoListener() {
    ...
    if (cachedVideoPlayerValue != null &&
        ...
        (cachedVideoPlayerValue.position) >=
            (cachedVideoPlayerValue?.duration)) {
